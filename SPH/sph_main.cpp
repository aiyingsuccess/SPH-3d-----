...
void set_shaders()//建立着色器
{
	char *vs=NULL;
	char *fs=NULL;

	vs=(char *)malloc(sizeof(char)*10000);//分配内存空间
	fs=(char *)malloc(sizeof(char)*10000);
	memset(vs, 0, sizeof(char)*10000);//内存空间初始化
	...
	f=glCreateShader(GL_FRAGMENT_SHADER);//创建片段着色器
	...
	glShaderSource(v, 1, &vv, NULL);//源代码字符数组绑定到对象
	glShaderSource(f, 1, &ff, NULL);

	int success;

	glCompileShader(v);//编译着色器顶点对象
	...
	p=glCreateProgram();//创建着色程序对象
	glAttachShader(p, v);//将顶点着色器对象附加到着色程序上
	glAttachShader(p, f);
	glLinkProgram(p);//对着色程序对象执行进行链接
	glUseProgram(p);//将OpenGL渲染管道切换到着色器模式，并使用刚才做好的（着色）程序对象
...
void init()
{
	...
	glViewport(0, 0, window_width, window_height);//设置OpenGL屏幕大小
...
void display_func()
{
	...
	glTranslatef(xTrans, yTrans, zTrans);//变换的是坐标系
...
void idle_func()// glut空闲处理回调函数   实现动画功能需要的函数
{
	glutPostRedisplay();//重新绘制
}

void reshape_func(GLint width, GLint height)// glut窗口重置回调函数 
{
	window_width=width;
...
void keyboard_func(unsigned char key, int x, int y)// glut键盘回调函数
{
	if(key == ' ')
	{
...
void mouse_func(int button, int state, int x, int y)// glut鼠标按下与释放回调函数 
{
	if (state == GLUT_DOWN)
	{
        buttonState = 1;
...
void motion_func(int x, int y)//glut鼠标移动回调函数
{
    float dx, dy;
    dx = (float)(x - ox);
    dy = (float)(y - oy);
